  ...
  for (const msg of messages) {
    if (msg.role === "user") {
      ...
      if (current.sources?.length) {
        lines.push("**Sources:**");
        current.sources.forEach((s, i) => lines.push(`- [${i+1}] p.${s.page} · ${s.type} — ${s.text?.slice(0,120)}...`));
        lines.push("");
      }
    }
  }
...
function downloadMarkdown(title, messages) {
  const blob = new Blob([buildMarkdown(title, messages)], { type: "text/markdown" });
...
function printAsPDF(title, messages) {
  ...
  w.document.write(`<!DOCTYPE html><html><head><title>${title}</title>
    ...
    p{margin:4px 0 12px}ul{margin:4px 0 12px;padding-left:20px}li{font-size:.85rem;color:#555}
...
function Tour({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="bg-background border border-border rounded-2xl shadow-xl p-6 w-full max-w-sm flex flex-col gap-4">
    ...
    <span className="relative inline-block">
      ...
      {visible && (
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-50
                        w-64 sm:w-72 bg-popover border border-border rounded-xl shadow-lg p-3 text-xs">
...
function CompareView({ versionA, versionB, streaming, onKeep, onClose }) {
  return (
    <div className="w-full border border-border rounded-2xl overflow-hidden mt-1">
      ...
      <div className="flex flex-col md:grid md:grid-cols-2 md:divide-x divide-border">
        <div className="p-4 flex flex-col gap-3 border-b md:border-b-0 border-border">
          <div className="flex items-center justify-between">
        ...
        <div className="p-4 flex flex-col gap-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => onKeep("B")} disabled={streaming}
              className="text-xs bg-primary text-primary-foreground px-2 py-1 rounded-lg hover:opacity-90 disabled:opacity-40">Keep B</button>
          </div>
          <div className="text-sm">
  ...
  return (
    <div className="flex flex-col gap-1 px-2 py-1">
      <input autoFocus
        ...
        placeholder="Describe the issue…" value={text}
...
function FeedbackButtons({ message, activeId }) {
  ...
  async function handleUp() {
    ...
    setSubmitting(true);
    try { await submitFeedback({ conversationId:activeId, messageIndex:message.messageIndex, versionIndex:currentVersion, rating:"up", reason:null }); setStatus("up"); }
    catch(e){console.error(e);} finally{setSubmitting(false);}
  }
  async function handleReason(reason) {
    setShowReasons(false); setSubmitting(true);
    try { await submitFeedback({ conversationId:activeId, messageIndex:message.messageIndex, versionIndex:currentVersion, rating:"down", reason }); setStatus("down"); }
    catch(e){console.error(e);} finally{setSubmitting(false);}
  }
...
function ThinkingIndicator({ stage }) {
  const stages = { retrieving:{label:"Searching documents…",width:"w-1/3"}, generating:{label:"Generating answer…",width:"w-2/3"} };
...
export default function App() {
  ...
  async function handleSelectConversation(id) {
    ...
    setMessages(convo.messages.map((m, i) => {
      if (m.role === "assistant") {
        return { id:i, role:"assistant", versions:m.versions||[{content:m.content||"",sources:m.sources||[]}],
          currentVersion:m.current_version??0, streaming:false, messageIndex:i,
  ...
  async function handleSend() {
    ...
    const assistantMsg = { id:assistantId, role:"assistant", versions:[{content:"",sources:[]}],
      currentVersion:0, streaming:true, messageIndex:null,
      compareMode:false, compareContent:"", compareSources:[], compareStreaming:false };

    setMessages((prev) => [...prev, userMsg, assistantMsg]);
    setInput(""); setIsLoading(true); setLoadingStage("retrieving");

    await streamChatQuery(query, convId, {
      onSources: (sources) => {
        setLoadingStage("generating");
        setMessages((prev) => prev.map((m) => m.id===assistantId ? {...m,versions:[{content:m.versions[0].content,sources}]} : m));
      },
      onToken: (token) => {
        setIsLoading(false);
        setMessages((prev) => prev.map((m) => m.id===assistantId ? {...m,versions:[{...m.versions[0],content:m.versions[0].content+token}]} : m));
      },
      onTitle: (title, cid) => {
      ...
      onDone: (msgIndex) => {
        setIsLoading(false);
        setMessages((prev) => prev.map((m) => m.id===assistantId ? {...m,streaming:false,messageIndex:msgIndex} : m));
      },
      onError: (err) => {
        setIsLoading(false);
        setMessages((prev) => prev.map((m) => m.id===assistantId ? {...m,versions:[{content:`Error: ${err}`,sources:[]}],streaming:false} : m));
      },
    });
  }
  ...
  async function handleRegenerate(messageId, action, versionIdx) {
    ...
    setMessages((prev) => prev.map((m) => m.id===messageId
      ? {...m,streaming:true,versions:[...m.versions,{content:"",sources:[]}],currentVersion:newVersionIdx} : m));
    setIsLoading(true); setLoadingStage("retrieving");

    await regenerateMessage(userMsg.content, activeId, assistantMsg.messageIndex, {
      onSources: (sources) => {
        setLoadingStage("generating");
        setMessages((prev) => prev.map((m) => { if(m.id!==messageId) return m; const v=[...m.versions]; v[newVersionIdx]={...v[newVersionIdx],sources}; return {...m,versions:v}; }));
      },
      onToken: (token) => {
        setIsLoading(false);
        setMessages((prev) => prev.map((m) => { if(m.id!==messageId) return m; const v=[...m.versions]; v[newVersionIdx]={...v[newVersionIdx],content:v[newVersionIdx].content+token}; return {...m,versions:v}; }));
      },
      onDone: () => { setIsLoading(false); setMessages((prev) => prev.map((m) => m.id===messageId ? {...m,streaming:false} : m)); },
      onError: (err) => {
        setIsLoading(false);
        setMessages((prev) => prev.map((m) => { if(m.id!==messageId) return m; const v=[...m.versions]; v[newVersionIdx]={content:`Error: ${err}`,sources:[]}; return {...m,versions:v,streaming:false}; }));
      },
    });
  }
  ...
  async function handleCompare(messageId, action) {
    if (action==="close") {
      setMessages((prev) => prev.map((m) => m.id===messageId ? {...m,compareMode:false,compareContent:"",compareSources:[],compareStreaming:false} : m));
      return;
    }
    const msgIdx = messages.findIndex((m) => m.id===messageId);
    if (msgIdx<1) return;
    ...
    setMessages((prev) => prev.map((m) => m.id===messageId ? {...m,compareMode:true,compareContent:"",compareSources:[],compareStreaming:true} : m));

    await regenerateMessage(userMsg.content, activeId, assistantMsg.messageIndex, {
      onSources: (sources) => setMessages((prev) => prev.map((m) => m.id===messageId ? {...m,compareSources:sources} : m)),
      onToken: (token) => setMessages((prev) => prev.map((m) => m.id===messageId ? {...m,compareContent:(m.compareContent||"")+token} : m)),
      onDone: () => setMessages((prev) => prev.map((m) => m.id===messageId ? {...m,compareStreaming:false} : m)),
      onError: (err) => setMessages((prev) => prev.map((m) => m.id===messageId ? {...m,compareContent:`Error: ${err}`,compareStreaming:false} : m)),
    });
  }
  ...
  function handleKeepVersion(messageId, side) {
    ...
    if (side==="A") {
      ...
      setMessages((prev) => prev.map((m) => m.id===messageId
        ? {...m,versions:newVersions,currentVersion:newVersionIdx,compareMode:false,compareContent:"",compareSources:[],compareStreaming:false} : m));
      if (msg.messageIndex!=null&&activeId) setMessageVersion(activeId, msg.messageIndex, newVersionIdx);
    }
  }
  ...
  const waitingForFirstToken = isLoading && messages.at(-1)?.role==="assistant" && messages.at(-1)?.versions?.[0]?.content==="";
      ...
      <div className="flex flex-col flex-1 min-w-0">
        ...
        <main className="flex-1 overflow-y-auto px-4 sm:px-6 py-6">
          <div className="max-w-4xl mx-auto flex flex-col gap-4">
            ...
            {messages.map((msg) => (
              <MessageBubble key={msg.id} message={msg} activeId={activeId}
